...
export function DataTableFilterMenu<TData>({
  ...
  // Use local state for filters instead of URL state (nuqs)
  // In a real Inertia app, you might want to sync this with the URL via router.get/visit
  // but for now we'll stick to local state passed via table prop
  const filters = (table.getState().columnFilters as ExtendedColumnFilter<TData>[]) || []
  ...
  const setFilters = React.useCallback(
    (
      ...
      table.setColumnFilters(updateOrValue)
    },
    [table]
  )
  ...
  const onFilterUpdate = React.useCallback(
    (filterId: string, updates: Partial<Omit<ExtendedColumnFilter<TData>, 'filterId'>>) => {
      debouncedSetFilters((prevFilters) => {
        const currentFilters = prevFilters as ExtendedColumnFilter<TData>[]
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
        ...
        >
          <HugeiconsIcon icon={FilterHorizontalIcon} className="text-muted-foreground" />
        ...
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger
            className={buttonVariants({
              ...
              size: 'sm',
              className:
                'h-full min-w-32 justify-start rounded-none border px-1.5 font-normal dark:bg-input/30',
            })}
            id={inputId}
            aria-controls={inputListboxId}
          >
            <HugeiconsIcon icon={Calendar01Icon} className="mr-2 size-3.5 shrink-0" />
          ...
          <PopoverContent id={inputListboxId} align="start" className="w-auto p-0">
            {filter.operator === 'isBetween' ? (
              <Calendar
                ...
                mode="range"
                selected={{
                  from: dateValue[0] ? new Date(Number(dateValue[0])) : undefined,
                  to: dateValue[1] ? new Date(Number(dateValue[1])) : undefined,
                }}
                onSelect={(date) => {
                  onFilterUpdate(filter.filterId, {
                    value: [
