...
(async () => {
  ...
  // 1. User info
  const user = await client.query('SELECT id, name, email, phone, status, activated_at, parent_id, leg FROM users WHERE id = 534707');
  console.log('=== USER ===');
  ...
  const dc = await client.query('SELECT COUNT(*)::int as total FROM users WHERE parent_id = 534707');
  console.log('\nDirect children: ' + dc.rows[0].total);

  // 3. Descendants with depth
  const desc = await client.query(`
    WITH RECURSIVE descendants AS (
      SELECT id, name, parent_id, leg, 1 as depth FROM users WHERE parent_id = 534707
      UNION ALL
  ...
  const tb = await client.query(`
    WITH RECURSIVE descendants AS (
      SELECT id FROM users WHERE parent_id = 534707
      UNION ALL
  ...
  const allTx = await client.query(`
    ...
    FROM transactions
    WHERE user_id = 534707 AND type = 'wallet_credit'
  ...
  const snapshots = await client.query(
    "SELECT * FROM monthly_income_snapshots WHERE user_id = 534707 ORDER BY id DESC"
  );
  ...
  // 8. Salaries
  const salaries = await client.query('SELECT * FROM salaries WHERE user_id = 534707 ORDER BY created_at DESC');
  ...
  // 9. Membership level income transactions
  const membershipLevel = await client.query(`
    SELECT * FROM transactions
    WHERE user_id = 534707
    AND remark LIKE '%Membership Level%'
    ORDER BY created_at DESC
  `);
  console.log('\n=== MEMBERSHIP LEVEL INCOME (' + membershipLevel.rows.length + ') ===');
  ...
  const wallets = await client.query(`
    SELECT income_wallet, working_wallet, repurchase_wallet, reward_wallet, total_invested FROM users WHERE id = 534707
  `);
  console.log('\n=== CURRENT WALLETS ===');
  ...
  const withdrawals = await client.query(`
    SELECT * FROM withdrawls WHERE user_id = 534707 ORDER BY created_at DESC
  `);
